...
| --- | --- | --- |
| OpenAI Images | `gpt-image-2`, `gpt-image-1.5`, `gpt-image-1`, `gpt-image-1-mini`, `dall-e-3` | `IMAGE_BASE_URL` + `IMAGE_API_KEY` (required; never reuse chat Qwen/etc.). GPT Image may need OpenAI org verification. |
| Gemini Nano Banana | `gemini-3.1-flash-image` (default), `gemini-3.1-flash-lite-image`, `gemini-3-pro-image`, `gemini-2.5-flash-image` | `GEMINI_API_KEY` (or `IMAGE_API_KEY`) |
