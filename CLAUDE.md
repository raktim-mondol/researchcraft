...
Some tool-facing features didn't survive the dsh migration cleanly and are tracked as open work, not silently dropped:
- The `interview` (blocking clarifying-questions form) and `modal_run` (remote Modal compute) tools aren't wired into the dsh agent yet — their HTTP-facing logic and state (`agent/interview.ts`, `agent/modal-tool.ts`) are intact, but the model can't call them. Both need state that lives in the main Fastify process (the interview answer map; a session's selected compute target) bridged into the dsh runtime SUBPROCESS, which is a separate OS process with its own memory — see the `TODO(#20)` comments in those files for the planned fix.
